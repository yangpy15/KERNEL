...
│   ├── common.py           # Shared columns and text utilities
...
│   ├── retrieval.py        # Text encoder for embedding retrieval
...
The exact system, first-pass recommendation, reassessment, final-rationale, and response-repair prompts are centralized in `src/prompts.py`.
...
| `src/common.py`            | Shared column definitions and text utilities.                                                                         |
...
Run `src/run_kernel.py` with the Stage 1 predictions, evaluation visits, authorized training partition, and external diagnosis knowledge-graph resources. The program builds the required evidence, loads the prompts from `src/prompts.py`, runs recommendation and reassessment, and writes the final recommendation and rationale.
...
This is the paper mainline: both diagnosis-KG files are supplied so that diagnosis-concept expansion and diagnosis-aware similar-case retrieval are enabled.

The implementation also permits a run without the external diagnosis KG as a method variant:
...
| `recommended_test_tags` | Final laboratory-category recommendations. | Cell Count;Electrolytes;Renal Function;Liver Function;Coagulation;Microbiology/Infection;Infection Markers;Blood Gas&Acid-Base;Cardiovascular;Metabolic                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### External MedlinePlus Final-Rationale Evidence

The mainline automatically loads the bundled structured MedlinePlus laboratory knowledge base from `data/MedlinePlus_kg.json`. After `recommended_test_tags` has been finalized, the program selects MedlinePlus evidence only for those final tags and supplies it to the final clinician-facing rationale call, which is enabled by default.
...
Use `evaluate.py` to calculate patient-level laboratory-category metrics on lab-positive visits and overall patient-level label-set metrics including `no_lab_recommended`. An empty `recommended_test_tags` set is represented as `no_lab_recommended` in the overall label-set evaluation.
